...
$train = 'train-uni-9mos-clean' . $i . '.txt';
$test = 'test-uni-9mos-clean' . $i . '.txt';
system("./../dpseg -C ../configs/config-dps.txt -o ../output_clean/english/$output --data-file ../corpora_clean/$train --eval-file ../corpora_clean/$test --ngram $ngram --a1 $a --b1 $b1 --b2 $b2 > ../output_clean/english/$stats");
}
